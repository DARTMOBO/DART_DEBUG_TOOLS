...
int CapacitiveSensor::SenseOneCycle(void)
{
	...
	*sOut |= sBit;         // set send Pin High
	...
	while ( !(*rIn & rBit)  && (total < 500 ) ) {  // while receive pin is LOW AND total is positive value
		total++;
	}
	...
	*sOut &= ~sBit;        // set send Pin LOW
	...
	while ( (*rIn & rBit) && (total < 500  ) ) {  // while receive pin is HIGH  AND total is less than timeout
		total++;
	}
