...
//  Serial.println(readingsXen[indexXen]);
 ...
 // Serial.println(cs_4_2.capacitiveSensorRaw(2048)/16);
 #endif
...
// Serial.println(cs_4_3.capacitiveSensorRaw(30000));
   ...
   // nella configurazione IKIZ le resistenze son due da 4.7m tra emettitore e plate, + una da 4.7m tra plate e ricevitore!
   // il risultato è :
   // side
   // fermo - 222 - 425
   // knob - 228 2200
   ...
   // knob 336 1900
   // touch  9800 10300
   // quindi metterei il limite a... 4096 e dividere per 32 - per avere 127
 #endif
...
lastbutton[numero] = 64; // 
  ...
  int valuepot;
  encoder_pot_calcolo(numero,minvalue[numero]-32);  // canale e velocità
  ...
  if (typetable[numero+page] > 159 )   // sotto il 160 sono note on e off
  { //encoder_pot_calcolo(numero2);
...
= (constrain(encodervaluepot[numero2] - (lastbutton[chan_]-64)*moltiplicatore,
0, 1023));  
// spegazione: si aggiunge il valore speed, positio o negativo a seconda del verso.
 ...
 {
  lightable[chan_]               
= (constrain( lightable[chan_]  - (lastbutton[chan_]-64)*moltiplicatore,
 0, 255));  
