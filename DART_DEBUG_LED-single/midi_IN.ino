...
if (valuetable[ledD]==note && bit_read(3,ledD) == 1   )  {
      ...
      if (page==0) { shifter.setPin(lightable[ledD],1); shifterwrite=1; // DmxSimple.write(dmxtable[ledD], velocity);
      }
...
bitWrite(scala[numero+2],i+7,  bitRead(maxvalue[encoder_mempos[numero]+max_modifiers] , i)    );
 ...
 void aux_preset() // preset di base caricato all'avvio soltanto se il preset sulla eeprom non è valido. 
 {
   for (byte i = 0; i <max_modifiers; i++) {
    modetable[i]=1;
    valuetable[i+page]=60; // di default, tutte le note emesse sono uguali. in questo modo, se non si montano multiplexers,  ogni in triggera otto note, che essendo tutte uguali verranno filtrate dal midiout in una solo nota.
    minvalue[i]=0;
    maxvalue[i]=127;
    typetable[i]=144;
    }
