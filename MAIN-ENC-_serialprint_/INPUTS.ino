...
//delay (100);
  ...
  if (auxx != 1) // se non c'ÃƒÂ¨ un preset nella eeprom
  {

    if (valore < upper_val)   /// se premo un pulsante - valore va a zero
   {
     detect_plexer();
    if (valore > 448 && valore < 576   ) // se si ha un valore intermedio
  {
   if (qwertyvalue[chan] == (valore /32) ) // se la lettura precedente ÃƒÂ¨ molto simile alla attuale (risoluzione 0-1024 downgradata a 0-32
 ...
 if (minvalue[general_mempos] > 0) 
  ...
  // if (channel >1)
     {
if (channel >4) scala_learn = -1; else scala_learn = 1; // mi serviva un variabile int non usata nel default preset... 

      if (valuetable[chan] == valuetable[chan+scala_learn]) // solo se non ÃƒÂ¨ ancora stata effettuata la diversificazione
     {
      if ((valore/128) <20 && qwertyvalue[chan+scala_learn]>30)  // se il valore attuale ÃƒÂ¨ diverso da un valore precedente all'interno dello stesso gruppo-plexer
        { // i valori di valuetable dentro il gruppo-plexer vanno resi tutti diversi
         { setPlexer(channel+scala_learn);
        // scala_learn = analogRead(plexer);
       // Serial.println(valore);

         // delay (200);
          if (  analogRead(plexer)> upper_val)  /// valori molto diversi
          {
            setPlexer(channel+(scala_learn*2));
